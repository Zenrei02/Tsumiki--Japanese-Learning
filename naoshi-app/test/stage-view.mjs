...
async function boot({ progress, depth } = {}) {
  ...
  w.console.warn = () => {};
  w.localStorage.setItem("naoshi-last-module", "grammar");
  if (progress) w.localStorage.setItem("n5-progress-v1", JSON.stringify(progress));
  if (depth) w.localStorage.setItem("learner-depth-v1", depth);
...
}
{
  ...
  if (!got) { fail("no way to dismiss the greeting"); }
    ...
    const saved = JSON.parse(w.localStorage.getItem("n5-progress-v1") || "{}");
