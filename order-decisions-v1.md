...
appears twice more: `sb-person` for 人·入·八, and `sb-root` (*"木 and 本 — one stroke apart"*)
and `sb-lid` (*"今 and 分 — the same lid"*) for the other two.
...
derived tail, 今·分 and 木·本, are exactly the pairs `sb-root` and `sb-lid` already cover.

> **Names corrected 2026-09-19 by the weekly audit.** Both paragraphs above said `sb-tree`.
> That was right when written: `sb-tree` was one lesson covering both pairs, and the v1→v2
> regroup split it into `sb-root` and `sb-lid`. It now survives only in `DEAD_IDS_V1` in
> `kanji-module.jsx`, as a source for migrating a learner's old progress — and that file
> records the deliberate choice that the two successors start fresh rather than inheriting
> its traces, "the drill is the point, not the ink." The reasoning here was always correct;
> only the lesson id had gone stale. The same sentences are generated by
> `kanji-ordering-pipeline-v2.py` into `kanji-order-report-v2.md` and were corrected in the
> same pass — **if you edit one, edit both**, or the next regeneration puts `sb-tree` back.
