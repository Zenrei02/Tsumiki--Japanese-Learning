# Stage 9 · Independence — skeleton v1

*Session 17. The N3 gap from `restage-and-stage3-gap-v1.md`, grouped into steps. Follows
the house format of `stage-2-skeleton-v1.md` and `stage-3-skeleton-v1.md`.*

## What this stage is for

Stage 8 taught politeness — the register dial. Stage 9 is the other half of N3, and its
theme is **carrying an argument**: saying why something happened and whose fault it was,
holding a topic at arm's length, drawing a conclusion, and grading a claim.

The through-line, and the thing that makes it a stage rather than a list:

> **Stage 4 finished the sentences. Stage 7 finished the conversation. Stage 9 is where a
> learner stops reporting and starts arguing** — because every pattern in it is about the
> speaker's stance toward a fact rather than about the fact itself.
...
roughly 55–60 lesson objects, which puts Stage 9 at the size of Stage 4.
...
Everything from `lesson-arc-design-v1.md` applies:

- **Required, not merely observed.** The arcs are situations a resident cannot avoid.
- **Hear-first where it fits** — several of these are recognition-first, because they are
  reading grammar before they are speaking grammar.
- Four-beat `exp`, examples, DEEP wrinkles, arcs with a flip and wrinkle extensions.
- Japanese at or below the step's own level; kanji outside `KANJI_DICT` flagged.
- **`くせに` needs a rudeness flag** in the same way `meirei` did — it is genuinely
  insulting and a learner who meets it in fiction will misjudge it.

## Milestone

`ms-n3` goes into Checkpoint 8, after `rc8`, matching `ms-n5` and `ms-n4`. It closes the
JLPT ladder the app actually teaches — S10 and S11 stay locked.

## Reviewer

Batch **M**. Lower risk than the keigo batch: this is grammar rather than claims about
what a room sounds like. The two items that need a native eye most are `kuseni`'s
rudeness calibration and `wakeda`, which almost every English explanation gets slightly
wrong by translating it as *reason*.
