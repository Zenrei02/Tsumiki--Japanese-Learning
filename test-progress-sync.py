...
already shipped that exact failure shape once — `tsumiki-n5-progress-v1` missing from
...
tsumiki-app/src/lib/sync.js AT RUN TIME, between the two marker comments, and
...
downstream of that single line — and green again on revert:
    ...
    FAIL: the differing key is named  (got [], wanted ["tsumiki-kanji-progress-v1"])
    FAIL: A CONFLICTED KEY MUST NOT APPEAR IN merged — that is silent data loss
    FAIL: a spent wallet is not an empty wallet
    FAIL: plain strings still compare  (got [], wanted ["tsumiki-kanji-mode"])
...
SYNC = HERE / "tsumiki-app" / "src" / "lib" / "sync.js"
