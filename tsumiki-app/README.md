...
Then, once, from inside this `tsumiki-app` folder:
...
> ⚠️ **The one thing that trips this up.** The app lives in `tsumiki-app/`, not at
> the repository root. Without a **base directory** of `tsumiki-app`, Netlify runs
...
Set it by hand: root/base directory `tsumiki-app`, build command `npm run build`,
...
**After either one**, from inside `tsumiki-app`: delete the `node_modules` folder
