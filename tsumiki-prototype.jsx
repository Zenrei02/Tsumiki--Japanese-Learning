...
function buildPlainReport({ text, result, context, furi }) {
  ...
  const L = [];
  L.push(`tsumiki — Japanese writing check`);
...
function buildAnkiTsv({ result, furi }) {
  ...
  return result.issues
    .map((iss) => {
      ...
      const tags = ["tsumiki", iss.category, iss.type, iss.jlpt].filter(Boolean).join(" ");
...
<title>tsumiki — ${esc(stamped())}</title>
<style>
...
<header><span class="logo">積</span><div><h1>tsumiki</h1>
...
${result.model_rewrite
  ...
  : ""}
<footer>Corrections are AI-generated and can be wrong. Generated by tsumiki.</footer>
</div></body></html>`;
}
...
// same sequence — tsumiki is a tool you press many times in a row.
...
export default function TsumikiPrototype() {
  ...
  const copyText = async (key, payload, filename = "tsumiki.txt") => {
    try {
  ...
  const downloadHtml = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `tsumiki-${fileStamp()}.html`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      setTimeout(() => URL.revokeObjectURL(url), 1000);
      ...
      setManual({ payload: html, filename: `tsumiki-${fileStamp()}.html` });
      flash("html", false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: T.paper, fontFamily: T.uiFont, color: T.ink }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 20px 80px" }}>
        ...
        <header style={{ display: "flex", alignItems: "baseline", gap: 14, marginBottom: 6 }}>
          ...
          <div>
            <div style={{ fontSize: 20, fontWeight: 600, letterSpacing: ".5px" }}>tsumiki</div>
        ...
        {result && segData && (
          <div style={{ marginTop: 28 }}>
            ...
            <div style={{ marginTop: 18, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              ...
              <button className="btn-ghost" onClick={() => copyText("text", buildPlainReport(exportArgs()), `tsumiki-${fileStamp()}.txt`)}>
              ...
              {result.issues.length > 0 && (
                <button className="btn-ghost" onClick={() => copyText("anki", buildAnkiTsv(exportArgs()), `tsumiki-${fileStamp()}.tsv`)}>
                  {saved?.key === "anki"
            ...
            <div style={{
              ...
              <p style={{ fontSize: 14, lineHeight: 1.6, margin: "0 0 12px" }}>
                ...
                do for us — especially the ones you think tsumiki got wrong. Copy your check and
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
                <button
                  ...
                  onClick={() =>
                    ...
                    }), `tsumiki-check-${submissionId}.txt`)
                  }
                >
              ...
              <p style={{ fontSize: 12, color: T.sub, margin: "10px 0 0", lineHeight: 1.6 }}>
                What gets copied: the Japanese you wrote, the corrections tsumiki gave, and your
